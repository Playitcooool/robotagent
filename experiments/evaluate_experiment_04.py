...
1. **任务完成度**：是否完成了提示中的核心任务？
   - 如果响应要求澄清关键参数（如起点位置、物体存在性等），且这些参数确实未在测试提示中提供，这是**合理的质疑**，不应扣分。
   - 只有当提示已提供足够信息但 agent 仍然不执行或瞎猜时，才扣分。

2. **正确性**：基于提示中给出的参数，结论是否正确？

3. **鲁棒性**：
   - 提示缺少必要信息 → 合理要求澄清 → 高分
   - 提示已提供足够信息 → 盲目执行或瞎猜 → 低分
   - 提示已提供足够信息 → 停下来问人但其实不需要 → 扣分
