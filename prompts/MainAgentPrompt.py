...
路由规则（按优先级）：
1. 纯问答 / 闲聊 → 直接回答。
2. 仿真任务（运动控制、物理场景、机器人操作）→ 调用 simulator，传递完整任务描述。
...
- simulator 会自动处理初始化（创建干净环境）和清理，不需要在任务描述中提醒

【关键】处理 simulator 返回结果时：
- simulator 返回的是 dict（如 {"status": "ok", "result": {"final_position": [...]}}），需要读取其中的具体字段
- 如果返回 "status": "error"，直接向用户报告错误，不要继续尝试
- 如果返回的是工具调用的原始输出（包含 "tool": "...", "message": "..."），从中提取关键结果
